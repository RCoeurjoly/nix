...
public:
     ...
     */
    Path toRealPath(const Path & path, const StringContext & context);
    ...
    std::string_view forceString(Value & v, StringContext & context, const PosIdx pos, std::string_view errorCtx);
...
public:
    ...
    std::optional<std::string> tryAttrsToString(const PosIdx pos, Value & v,
        StringContext & context, bool coerceMore = false, bool copyToStore = true);
     ...
     */
    BackedStringView coerceToString(const PosIdx pos, Value & v, StringContext & context,
        std::string_view errorCtx,
        bool coerceMore = false, bool copyToStore = true,
        bool canonicalizePath = true);

    StorePath copyPathToStore(StringContext & context, const SourcePath & path);
     ...
     */
    SourcePath coerceToPath(const PosIdx pos, Value & v, StringContext & context, std::string_view errorCtx);

    /**
     * Like coerceToPath, but the result must be a store path.
     */
    StorePath coerceToStorePath(const PosIdx pos, Value & v, StringContext & context, std::string_view errorCtx);
     ...
     * element, which is either a `StringContextElem::Opaque` or
     * `StringContextElem::Built`. (`StringContextEleme::DrvDeep`
     * is not permitted).
     *
...
public:
     ...
     * single `StringContextElem::Opaque` element of that store path.
     */
     ...
     *
     * The string is the printed store path with a context containing a
     * single `StringContextElem::Built` element of the drv path and
     * output name.
     *
     ...
     */
    [[nodiscard]] StringMap realiseContext(const StringContext & context, StorePathSet * maybePaths = nullptr, bool isIFD = true);
