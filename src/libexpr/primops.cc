...
StringMap EvalState::realiseContext(const StringContext & context, StorePathSet * maybePathsOut, bool isIFD)
{
...
static void import(EvalState & state, const PosIdx pos, Value & vPath, Value * vScope, Value & v)
{
    ...
    if (auto storePath = isValidDerivationInStore()) {
        ...
        attrs.alloc(state.sDrvPath).mkString(path2, {
            StringContextElem::DrvDeep { .drvPath = *storePath },
        });
    ...
    Value & v)
{
    ...
    result.alloc(state.sDrvPath).mkString(drvPathS, {
        StringContextElem::DrvDeep { .drvPath = drvPath },
    });
...
static void prim_storePath(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    context.insert(StringContextElem::Opaque { .path = path2 });
...
static void prim_toFile(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto c : context) {
        if (auto p = std::get_if<StringContextElem::Opaque>(&c.raw))
            refs.insert(p->path);
        else
            state.error<EvalError>(
...
static void prim_hashString(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    StringContext context; // discarded
