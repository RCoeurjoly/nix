...
Path EvalState::toRealPath(const Path & path, const StringContext & context)
{
...
static const char * * encodeContext(const StringContext & context)
{
    if (!context.empty()) {
        size_t n = 0;
        auto ctx = (const char * *)
...
void Value::mkString(std::string_view s, const StringContext & context)
{
    mkString(makeImmutableString(s), encodeContext(context));
}

void Value::mkStringMove(const char * s, const StringContext & context)
{
...
void EvalState::mkStorePathString(const StorePath & p, Value & v)
{
    v.mkString(
        ...
        StringContext {
            StringContextElem::Opaque { .path = p },
        });
}
    ...
    Value & v)
{
    v.mkString(
        ...
        StringContext {
            std::visit([](auto && v) -> StringContextElem { return v; }, p),
        });
}
...
void copyContext(const Value & v, StringContext & context)
{
    if (v.payload.string.context)
        for (const char * * p = v.payload.string.context; *p; ++p)
            context.insert(StringContextElem::parse(*p));
}


std::string_view EvalState::forceString(Value & v, StringContext & context, const PosIdx pos, std::string_view errorCtx)
{
...
std::optional<std::string> EvalState::tryAttrsToString(const PosIdx pos, Value & v,
    StringContext & context, bool coerceMore, bool copyToStore)
{
...
StorePath EvalState::copyPathToStore(StringContext & context, const SourcePath & path)
{
    ...
    context.insert(StringContextElem::Opaque {
        .path = dstPath
    });
    return dstPath;
}
...
SourcePath EvalState::coerceToPath(const PosIdx pos, Value & v, StringContext & context, std::string_view errorCtx)
{
    try {
        forceValue(v, pos);
    } catch (Error & e) {
...
StorePath EvalState::coerceToStorePath(const PosIdx pos, Value & v, StringContext & context, std::string_view errorCtx)
{
    auto path = coerceToString(pos, v, context, errorCtx, false, false, true).toOwned();
...
std::pair<SingleDerivedPath, std::string_view> EvalState::coerceToSingleDerivedPathUnchecked(const PosIdx pos, Value & v, std::string_view errorCtx)
{
    ...
    auto derivedPath = std::visit(overloaded {
        [&](StringContextElem::Opaque && o) -> SingleDerivedPath {
            return std::move(o);
        },
        [&](StringContextElem::DrvDeep &&) -> SingleDerivedPath {
            error<EvalError>(
                "string '%s' has a context which refers to a complete source and binary closure. This is not supported at this time",
                s).withTrace(pos, errorCtx).debugThrow();
        },
        [&](StringContextElem::Built && b) -> SingleDerivedPath {
            return std::move(b);
        },
    }, ((StringContextElem &&) *context.begin()).raw);
    return {
...
std::string ExternalValueBase::coerceToString(EvalState & state, const PosIdx & pos, StringContext & context, bool copyMore, bool copyToStore) const
{
