...
static void prim_unsafeDiscardOutputDependency(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    StringContext context2;
    for (auto && c : context) {
        if (auto * ptr = std::get_if<StringContextElem::DrvDeep>(&c.raw)) {
            context2.emplace(StringContextElem::Opaque {
                .path = ptr->drvPath
            });
        } else {
...
static void prim_addDrvOutputDependencies(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    StringContext context2 {
        (StringContextElem { std::visit(overloaded {
            [&](const StringContextElem::Opaque & c) -> StringContextElem::DrvDeep {
            ...
            [&](const StringContextElem::Built & c) -> StringContextElem::DrvDeep {
                state.error<EvalError>(
                    "`addDrvOutputDependencies` can only act on derivations, not on a derivation output such as '%1%'",
                    c.output
                ).atPos(pos).debugThrow();
            },
            [&](const StringContextElem::DrvDeep & c) -> StringContextElem::DrvDeep {
...
static void prim_getContext(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto && i : context) {
        std::visit(overloaded {
            [&](StringContextElem::DrvDeep && d) {
                contextInfos[std::move(d.drvPath)].allOutputs = true;
            },
            [&](StringContextElem::Built && b) {
            ...
            [&](StringContextElem::Opaque && o) {
                contextInfos[std::move(o.path)].path = true;
            },
        }, ((StringContextElem &&) i).raw);
    }
    ...
    for (auto & i : *args[1]->attrs()) {
        ...
        if (auto attr = i.value->attrs()->get(sPath)) {
            if (state.forceBool(*attr->value, attr->pos, "while evaluating the `path` attribute of a string context"))
                context.emplace(StringContextElem::Opaque {
                    .path = namePath,
                });
        }
        ...
        if (auto attr = i.value->attrs()->get(sAllOutputs)) {
            if (state.forceBool(*attr->value, attr->pos, "while evaluating the `allOutputs` attribute of a string context")) {
                ...
                context.emplace(StringContextElem::DrvDeep {
                    .drvPath = namePath,
                });
            }
        }
        ...
        if (auto attr = i.value->attrs()->get(state.sOutputs)) {
            ...
            for (auto elem : attr->value->listItems()) {
                ...
                context.emplace(StringContextElem::Built {
