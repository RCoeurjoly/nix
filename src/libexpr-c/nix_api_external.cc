...
nix_err nix_external_add_string_context(nix_c_context * context, nix_string_context * ctx, const char * c)
{
    if (context)
        context->last_err_code = NIX_OK;
    try {
        auto r = nix::StringContextElem::parse(c);
        ctx->ctx.insert(r);
    }
    NIXC_CATCH_ERRS
}
...
public:
    ...
    virtual nlohmann::json printValueAsJSON(
        nix::EvalState & state, bool strict, nix::StringContext & context, bool copyToStore = true) const override
    {
